...
public class OrderServiceImpl implements OrderService {
        ...
        for (ShoppingCart cart : shoppingCartList) {
            if (cart.getDishId() != null) {
                ...
                if (dish == null || dish.getStatus().equals(StatusConstant.DISABLE)) {
                    throw new OrderBusinessException(MessageConstant.DISH_UNAVAILABLE() + cart.getName());
                }
                ...
                if (setmeal == null || setmeal.getStatus().equals(StatusConstant.DISABLE)) {
                    throw new OrderBusinessException(MessageConstant.SETMEAL_UNAVAILABLE() + cart.getName());
                }
            }
        }
    ...
    @Value("${delivo.shop.address}")
    private String shopAddress;

    @Value("${delivo.baidu.ak}")
    private String ak;
    ...
    private void checkOutOfRange(String address) {
        ...
        if (!jsonObject.getString("status").equals("0")) {
            throw new OrderBusinessException(MessageConstant.STORE_ADDRESS_PARSE_FAILED());
        }
        ...
        if (!jsonObject.getString("status").equals("0")) {
            throw new OrderBusinessException(MessageConstant.DELIVERY_ADDRESS_PARSE_FAILED());
        }
        ...
        if (!jsonObject.getString("status").equals("0")) {
            throw new OrderBusinessException(MessageConstant.ROUTE_PLAN_FAILED());
        }
    ...
    public OrderPaymentVO mockPayment(OrdersPaymentDTO ordersPaymentDTO) {
        // 直接触发内部的支付成功业务流转，把状态变更为已支付、代接单，播报WebSocket
        paySuccess(ordersPaymentDTO.getOrderNumber());

        // 返回一个虚拟的支付参数，前端仅做流程过渡
    ...
    public void confirm(OrdersConfirmDTO ordersConfirmDTO) {
        ...
        map.put("type", 4); // 4表示接单通知
        map.put("orderId", ordersConfirmDTO.getId());
        map.put("content", "订单号：" + ordersDB.getNumber());
        String userSid = String.valueOf(ordersDB.getUserId());
        webSocketServer.sendToClient(userSid, JSON.toJSONString(map));
    }
    ...
    public void rejection(OrdersRejectionDTO ordersRejectionDTO) throws Exception {
        ...
        if (payStatus == Orders.PAID) {
            log.info("用户已支付，订单号：{}，跳过退款（未接入微信支付）", ordersDB.getNumber());
        }
        ...
        map.put("type", 3);
        map.put("orderId", ordersDB.getId());
        map.put("reason", ordersRejectionDTO.getRejectionReason());
        String userSid = String.valueOf(ordersDB.getUserId());
        log.info("拒单通知：向用户 sid={} 发送消息 {}", userSid, JSON.toJSONString(map));
        webSocketServer.sendToClient(userSid, JSON.toJSONString(map));
    }
